  ...
  return <>
    <section className={c.hero}>
      <div><span className={s.kicker}>{en ? 'Rhema AI / AI and software for business' : 'Rhema AI / Внедрение AI и разработка для бизнеса'}</span>
        ...
        <p className={c.lead}>{en ? 'On a free discovery call, we review your task, suggest what to automate and explain where to start. We build the solution if you decide to move forward.' : 'На бесплатной диагностике разберём вашу задачу, покажем, что можно автоматизировать, и объясним, с чего начать. Если решите двигаться дальше — разработаем и внедрим решение.'}</p>
        <div className={s.actions}><a className={s.button} href="#contact">{en ? 'Get a free diagnosis' : 'Получить бесплатную диагностику'}<span aria-hidden="true">↗</span></a><a className={s.textLink} href="#cases">{en ? 'See our projects' : 'Посмотреть наши проекты'} ↓</a></div>
        <span className={s.note}>{en ? 'A call with our team · No technical brief · No obligation to buy' : 'Созвон с командой · Без технического задания · Без обязательства покупать'}</span>
      </div>
