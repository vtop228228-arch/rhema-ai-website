  ...
  return <section className={s.contact} id="contact" aria-labelledby="contact-title">
    ...
    <div className={s.formShell}><div className={s.formCore}>
      ...
      <form data-analytics-private onSubmit={submit} className={s.form} aria-busy={status === 'sending'} onChange={() => { if (!started.current) { ymGoal('contact_start', { page: window.location.pathname }); started.current = true; } }}>
        ...
        <label className={s.consent}><input type="checkbox" name="consent" required aria-invalid={invalidField === 'consent' || undefined} aria-describedby={invalidField === 'consent' ? 'project-error' : undefined} /><span>{en ? 'I give my ' : 'Даю '}<Link href={en ? '/en/consent' : '/consent'} target="_blank" rel="noopener noreferrer">{en ? 'consent to the processing of my personal data' : 'согласие на обработку персональных данных'}</Link>.</span></label>
