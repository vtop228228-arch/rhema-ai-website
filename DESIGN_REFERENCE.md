# Актуальный дизайн — Editorial / сентябрь 2026

Источник истины для публичных страниц: `src/components/marketing/Editorial.module.css` и `src/components/layout/Chrome.module.css`.
Референс направления: https://pay.larkinmd7.ru/ . Услуги, тексты и структура адаптированы под Rhema AI.
Тёплый светлый фон #f4f2ec, текст #18201e, синий #1649e8, лаймовый #d9ef88, Manrope с кириллицей.
Новая структура: предложение → задачи → проекты → процесс → форматы и стоимость → FAQ → AI-диагностика → контакт.
Карточки имеют вложенную рамку, навигация и CTA — скругления, движение учитывает prefers-reduced-motion.
Проекты описываются через реализованные функции. Неподтверждённые финансовые показатели и гарантии результата не публикуем.
Старые компоненты и токены ниже сохранены для истории и демонстрационной страницы; не использовать их как эталон новых публичных страниц.
